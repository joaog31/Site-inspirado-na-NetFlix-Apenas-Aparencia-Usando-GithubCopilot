...
/* Renderiza as secoes de series no container principal */
...
/* Controlador principal da pagina de series */
class SeriesApp {
    constructor(profileStorage, headerView, seriesRenderer) {
    ...
    init(seriesData) {
        ...
        this.seriesRenderer.render(seriesData);
    }
    ...
    renderOnlyItem(item) {
        ...
        this.seriesRenderer.render([{ title: 'Resultado da busca', items: [item] }]);
    }
}
...
function setupSearch(seriesApp) {
    ...
    return inicializarBusca({
        ...
        onSelectResult: (item) => seriesApp.renderOnlyItem(item)
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const seriesApp = new SeriesApp(profileStorage, headerView, seriesRenderer);
